...
zero_prior_pb = (label_train[label_train == 0].count()) / numberofelement
...
def tokenize(message):
   ...
   # print(all_words)
    all_wordss = [word.lower() for word in all_words if word.isalpha()]
   # print(all_wordss)
    all_words_lemmate = [ps.stem(lemmatizer.lemmatize(w)) for w in all_wordss]
...
def calculate_error_zero(test):
    probability_zero = 1
    smooth_zero = 1/(len(label_train_zero_class) + len(label_train))
